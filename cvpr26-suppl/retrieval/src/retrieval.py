...
def compute_retrieval(gallery_eval_type: str, query_eval_type: str, dataroot: Path, dataset_name: str,
    ...
    if kwargs.get('project', None):
        projection = span_projection.SpanProjection(
            clip_model=clip_model,
            tokenize_fn=clip.tokenize if isinstance(clip_model, CLIP) else clip_model.tokenizer,
            span_feats_cache=os.path.join('data', 'text_features',clip_model_name, dataset_name),
            device=device)
        gallery_features = projection.project(gallery_features)
        query_features = projection.project(query_features)
    ...
    RAM_LIMIT = 10e9  # don't occupy more than 10GB RAM
    expected_ram_size = len(gallery_features) * len(query_features) * 4
    chunks = int(1 + (expected_ram_size // RAM_LIMIT))
    chunk_size = len(query_features) // chunks
    print(f'Calculating metrics in {chunks} chunks')
    metrics_chunks = []
    for i in range(chunks):
        start, end = i * chunk_size, (i + 1) * chunk_size
        print(f'{start}:{end}')
        similarities = calculate_similarities(dataset_name, gallery_features, query_features[start:end],
                                              kwargs.get('split_size', 32))
        print(similarities.shape)

        m = (get_retrieval_metrics(dataset_name, similarities, query_labels[start:end], gallery_labels,
    ...
    # merge chunks back
    metrics = {k: [] for k in metrics_chunks[0].keys()}
    for m in metrics_chunks:
        for k, v in m.items():
            metrics[k].append(v)
    chunk_weights = np.array(
        [chunk_size] * (chunks - 1) + [len(similarities)])  # re-balance that last chunk has less samples
    chunk_weights = chunk_weights / chunk_weights.sum()
    print(chunk_weights)
    metrics = {k: (chunk_weights * np.array(v)).sum() for (k, v) in metrics.items()}
...
def calculate_similarities(dataset_name: str, gallery_features: torch.Tensor, query_features: torch.Tensor,
    ...
    if dataset_name == 'imagenet':  # For imagenet we use float16 to save CPU memory
        similarities = torch.empty((query_features.shape[0], gallery_features.shape[0]), device='cpu',
                                   dtype=torch.float16) # this would still be ~ 128GB 50000x1281166...
        # so instead of accumulating all similarities we measure in different splits and then average AP
        # since ap is anyway calculated only per-query and averaged in the end
...
def compute_roxford_rparis_metrics(dataset_name: str, similarities: torch.Tensor):
    ...
    gnd = ROxfordRParisDataset.get_ground_truth(dataset_name)['gnd']
...
def compute_metrics(dataset_name: str,
                    similarities: torch.Tensor,
                    query_labels: torch.Tensor,
                    gallery_labels: torch.Tensor,
                    is_query_gallery_split_same: bool = False):
    ...
    for query in tqdm(range(num_queries), desc='Computing retrieval metrics'):
        ...
        if is_query_gallery_split_same:  # Remove the query image from the gallery set
            ...
            # or mask = identity_idx = query_sim.argmax() when using chunked (and thus not aligned by idx) inputs
        ...
        # Recall at 1
        recall_at_1.append(query_true[query_sim.argmax()].int().item()) # 0 or 1
...
def add_args_to_parser():
    ...
    parser.add_argument("--query_eval_type", type=str,
                        ...
                        default='image', help="Type of feature used for query:\n"
    ...
    parser.add_argument("--gallery_eval_type", type=str,
                        choices=['oti', 'ovi', 'image', 'text'],
                        default='image', help="Type of feature used for gallery:\n"
                                            "'oti' for OTI-inverted features,\n"
                                            "'ovi' for OVI-inverted features,\n"
                                            "'image' for original image encoder features,\n"
                                            "'text' for original text encoder features.")
    ...
    parser.add_argument("--project", action="store_true", help="Whether to project the features on semantic imagenet axes")
