...
from src.models.siglip2 import load_siglip
...
def load_clip(clip_model_name, open_clip_pretrained="", use_open_clip=False,
    ...
    if "dino" in clip_model_name.lower():
        print("Loading DINO model: ", clip_model_name)
        # must have an encode_image function
        clip_model, clip_preprocess = load_dino(clip_model_name, device=local_device)
    elif "siglip" in clip_model_name.lower():
        print("Loading SigLIP model: ", clip_model_name)
        clip_model, clip_preprocess = load_siglip(clip_model_name, device=local_device)

    elif "SLIP" in clip_model_name:
    ...
    if 'dino' not in clip_model_name and 'siglip' not in clip_model_name:
