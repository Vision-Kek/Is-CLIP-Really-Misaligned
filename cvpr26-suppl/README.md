...
```
Done. Now you can replace the original CLIP `image_embeddings` with the resulting `image_embeddings_proj` for better performance in classification and retrieval tasks.

### Similarity Recovery (Appendix A)

With the following lines of code, recovery of image-image similarities given only text-image similarities:
```python
import torch
import torch.nn.functional as F
...
You can have a look at [s_intra_recovery.py](s_intra_recovery.py) for more analysis.

It walks you through the procedure of recovering $S_{intra}$ from $S_{inter}$ from Appendix A step by step.


## Run Few-Shot Classification

See [here](few_shot_classification/README.md).

## Run Retrieval

See [here](retrieval/README.md).